...
//          DONE (2) Use NotificationCompat.Builder to begin building the notification
            NotificationCompat.Builder weatherNotificationBuilder = new NotificationCompat.Builder(context)
...
//          DONE (3) Create an Intent with the proper URI to start the DetailActivity
            Intent todaysWeatherIntent = new Intent(context, DetailActivity.class);
            todaysWeatherIntent.setData(todaysWeatherUri);

//          DONE (4) Use TaskStackBuilder to create the proper PendingIntent
            TaskStackBuilder taskStackBuilder = TaskStackBuilder.create(context);
            taskStackBuilder.addNextIntentWithParentStack(todaysWeatherIntent);

            PendingIntent pendingIntent = taskStackBuilder.getPendingIntent(0, PendingIntent.FLAG_UPDATE_CURRENT);

//          DONE (5) Set the content Intent of the NotificationBuilder
            weatherNotificationBuilder.setContentIntent(pendingIntent);
...
//          DONE (7) Notify the user with the ID WEATHER_NOTIFICATION_ID
            notificationManager.notify(NOTIFICATION_ID, weatherNotificationBuilder.build());
