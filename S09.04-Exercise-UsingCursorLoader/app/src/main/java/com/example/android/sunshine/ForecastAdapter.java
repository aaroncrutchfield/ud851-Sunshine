...
//      DONE (7) Generate a weather summary with the date, description, high and low
        long dateInMilis = mCursor.getLong(MainActivity.INDEX_DATE);
        ...
        double highInCelsius = mCursor.getDouble(MainActivity.INDEX_MAX_TEMP);
        double lowInCelsius = mCursor.getDouble(MainActivity.INDEX_MIN_TEMP);

        String highAndLowTemp = SunshineWeatherUtils.formatHighLows(mContext, highInCelsius, lowInCelsius);

        String summaryString = dateString + " - " + description + " - " + highAndLowTemp;

//      DONE (8) Display the summary that you created above
        forecastAdapterViewHolder.weatherSummary.setText(summaryString);
    }
